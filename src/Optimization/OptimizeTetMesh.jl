...
# ----------------------------
function compute_tet_quality(
    v1::SVector{3,Float64},
    v2::SVector{3,Float64},
    v3::SVector{3,Float64},
    v4::SVector{3,Float64},
)
    # Calculate edge vectors
    edges = [v2 - v1, v3 - v1, v4 - v1, v3 - v2, v4 - v2, v4 - v3]
...
# ----------------------------
function would_cause_inversion(
    mesh::BlockMesh,
    node_idx::Int,
    new_position::SVector{3,Float64},
)
...
function laplacian_smooth!(
    ...
    for iter = 1:max_iterations
        ...
        for node_idx = 1:length(mesh.X)
            ...
            new_position =
                (1.0 - damping_factor) * mesh.X[node_idx] + damping_factor * barycenter
            ...
            if would_cause_inversion(mesh, node_idx, new_position)
                ...
                new_position =
                    (1.0 - reduced_damping) * mesh.X[node_idx] +
                    reduced_damping * barycenter
            ...
            current_min_quality = minimum(
                compute_element_quality(mesh, elem_idx) for elem_idx in mesh.INE[node_idx]
            )
            ...
            # Compute new quality
            new_min_quality = minimum(
                compute_element_quality(mesh, elem_idx) for elem_idx in mesh.INE[node_idx]
            )

            # Revert if quality decreases below threshold
            if new_min_quality < quality_threshold ||
               new_min_quality < current_min_quality * 0.9
        ...
        println(
            "  Iteration $iter: Max displacement = $max_displacement, Nodes moved = $nodes_moved",
        )
...
# ----------------------------
function optimize_mesh_adaptive!(
    mesh::BlockMesh;
    max_iterations::Int = 20,
    initial_damping::Float64 = 0.5,
    min_damping::Float64 = 0.1,
    quality_threshold::Float64 = 0.01,
    tolerance::Float64 = 1e-6,
)
    ...
    element_qualities = [compute_element_quality(mesh, i) for i = 1:length(mesh.IEN)]
    ...
    println(
        "  Initial mesh quality: Min = $initial_min_quality, Avg = $initial_avg_quality",
    )
    ...
    for iter = 1:max_iterations
        ...
        for node_idx in node_indices
            ...
            if would_cause_inversion(mesh, node_idx, new_position)
                ...
                for _ = 1:10  # Limit binary search iterations
                    mid = (low + high) / 2
    ...
    final_qualities = [compute_element_quality(mesh, i) for i = 1:length(mesh.IEN)]
    final_min_quality = minimum(final_qualities)
    final_avg_quality = sum(final_qualities) / length(final_qualities)
    quality_improvement =
        (final_avg_quality - initial_avg_quality) / initial_avg_quality * 100
...
# ----------------------------
function optimize_surface_nodes!(
    mesh::BlockMesh,
    scheme::String;
    max_iterations::Int = 10,
    damping_factor::Float64 = 0.3,
    tolerance::Float64 = 1e-6,
)
    ...
    for iter = 1:max_iterations
        ...
        for node_idx in surface_nodes
            ...
            barycenter =
                sum(mesh.X[n] for n in adjacent_surface_nodes) /
                length(adjacent_surface_nodes)
...
function optimize_mesh!(
    ...
    laplacian_smooth!(
        mesh,
        max_iterations = interior_iterations,
        damping_factor = damping_factor,
    )

    # Then optimize the surface nodes while preserving the isosurface
    optimize_surface_nodes!(
        mesh,
        scheme,
        max_iterations = surface_iterations,
        damping_factor = damping_factor*0.6,
    )

    # Finally, apply adaptive optimization with quality-based damping
    optimize_mesh_adaptive!(
        mesh,
        max_iterations = adaptive_iterations,
        initial_damping = damping_factor,
    )
