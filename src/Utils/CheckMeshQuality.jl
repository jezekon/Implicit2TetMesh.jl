...
function compute_quality_metrics!(mesh::BlockMesh)
    ...
    for tet in mesh.IEN
        ...
        # Compute edges
        edges = [vertices[j] - vertices[i] for i = 1:3 for j = (i+1):4]
        ...
        dihedral_angles = [
            acos(clamp(dot(face_normals[i], face_normals[j]), -1.0, 1.0)) for i = 1:3
            for j = (i+1):4
        ]
        ...
        surface_area = sum([
            0.5 * norm(cross(vertices[j]-vertices[i], vertices[k]-vertices[i])) for
            (i, j, k) in [(1, 2, 3), (1, 2, 4), (1, 3, 4), (2, 3, 4)]
        ])

        # Mean ratio quality metric (normalized shape measure)
        mean_ratio =
            12.0 * (3.0 * volume)^(2.0/3.0) / sum([dot(edge, edge) for edge in edges])
...
function verify_mesh_intersections(mesh::BlockMesh)
    ...
    vertex_sharing_elements = [Set{Int}() for _ = 1:length(mesh.IEN)]

    # For each vertex, store a list of elements that contain it
    vertex_to_tets = [Vector{Int}() for _ = 1:length(mesh.X)]
    for (tet_idx, tet) in enumerate(mesh.IEN)
        for v in tet
    ...
    neighbors = [Set{Int}() for _ = 1:length(mesh.IEN)]
    ...
    tet_aabbs =
        Vector{Tuple{SVector{3,Float64},SVector{3,Float64}}}(undef, length(mesh.IEN))
    ...
    function aabb_intersect(box1, box2)
        ...
        return all(
            box1[1][i] - tol <= box2[2][i] && box1[2][i] + tol >= box2[1][i] for i = 1:3
        )
    end
...
function compute_all_dihedral_angles(mesh::BlockMesh)
    ...
    for tet in mesh.IEN
        ...
        for i = 1:4
            face_center =
                (
                    vertices[setdiff(1:4, [i])[1]] +
                    vertices[setdiff(1:4, [i])[2]] +
                    vertices[setdiff(1:4, [i])[3]]
                ) / 3
        ...
        tet_angles = [
            acos(-clamp(dot(face_normals[i], face_normals[j]), -1.0, 1.0)) * 180/π for
            i = 1:3 for j = (i+1):4
        ]
...
function plot_dihedral_angles_histogram(angles::Vector{Float64}, output_path::String)
    ...
    text!(
        ax,
        90,
        0.9*maximum(ax.finallimits.val.origin[2] + ax.finallimits.val.widths[2]),
...
function assess_mesh_quality(mesh::BlockMesh, output_prefix::String)
    ...
    println(
        "  Aspect ratio: range [$(quality_metrics["aspect_ratio"].minimum), $(quality_metrics["aspect_ratio"].maximum)], mean $(quality_metrics["aspect_ratio"].mean)",
    )
    println(
        "  Min dihedral angle: range [$(quality_metrics["min_dihedral_angle"].minimum), $(quality_metrics["min_dihedral_angle"].maximum)], mean $(quality_metrics["min_dihedral_angle"].mean)",
    )
    println(
        "  Shape quality: range [$(quality_metrics["shape_quality"].minimum), $(quality_metrics["shape_quality"].maximum)], mean $(quality_metrics["shape_quality"].mean)",
    )
    println()
    ...
    println(
        "  Angles near ideal (65.53°-75.53°): $angles_near_ideal ($percent_near_ideal%)",
    )
    ...
    println(
        "  Intersecting element pairs: $(intersection_check["stats"]["intersecting_pairs"])",
    )
    println()
