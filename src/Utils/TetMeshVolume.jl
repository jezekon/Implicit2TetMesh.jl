...
function TetMesh_volumes(
    mesh::BlockMesh;
    volume_threshold::Union{Float64,Nothing} = nothing,
)
    ...
    if !isempty(large_elements)
        ...
        for (i, (elem_id, volume)) in
            enumerate(sorted_large[1:min(5, length(sorted_large))])
    ...
    if !isempty(negative_volumes)
        ...
        for (i, (elem_id, volume)) in
            enumerate(sorted_negative[1:min(5, length(sorted_negative))])
            println(@sprintf("| %-10d | %-22.6e |", elem_id, volume))
        end
