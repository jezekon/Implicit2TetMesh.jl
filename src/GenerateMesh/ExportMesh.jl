...
function export_mesh_vtu_quality(mesh::BlockMesh, filename::String)
    ...
    @inbounds for i = 1:nelements
        ...
        dihedral_angles = [
            acos(-clamp(dot(normals[i], normals[j]), -1.0, 1.0)) * 180 / π for i = 1:3
            for j = (i+1):4
        ]
    ...
    @inbounds for i = 1:nelements
        ...
        elseif jacobian_dets[i] < threshold_det
            ...
            # (threshold = 0.5, max = 1)
            jacobian_quality[i] =
                0.5 + 0.5 * min(jacobian_dets[i] / (10 * threshold_det), 1.0)
        end
    ...
    if negative_jacobian_count > 0
        println(
            "WARNING: Found $negative_jacobian_count elements with negative Jacobian determinant!",
        )
    ...
    println(
        "Identified $problem_count problematic elements out of $nelements total ($(round(100*problem_count/nelements, digits=2))%).",
    )
    ...
    println(
        "1. Load the file and select 'Surface With Edges' for better element visibility",
    )
    println(
        "2. Use 'problematic' field with threshold filter set to 1 to isolate problematic elements",
    )
